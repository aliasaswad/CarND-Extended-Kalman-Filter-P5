...
The measurement data for each line gets pushed onto a `measurement_pack_list`. The ground p <sub>x</sub> truth `[p <sub>x</sub>, p <sub>y</sub>, v <sub>x</sub>, v <sub>y</sub>]` for each line in the data file gets pushed ontoground_truthso RMSE can be calculated later from tools.cpp.
